...
describe('KnowledgeNode', () => {
  ...
  describe('落叶化土', () => {
    it('isCompost：热力低于阈值应返回 true', () => {
      const node = new KnowledgeNode({ node_id: 'c1', axis: '生', state: '实', summary: 'test', heat_score: 0.2 });
      assert.strictEqual(node.isCompost(), true);
    });

    it('isCompost：热力等于阈值不应化土', () => {
      const node = new KnowledgeNode({ node_id: 'c2', axis: '业', state: '实', summary: 'test', heat_score: 0.3 });
      assert.strictEqual(node.isCompost(), false);
    });

    it('isCompost：热力高于阈值应返回 false', () => {
      const node = new KnowledgeNode({ node_id: 'c3', axis: '思', state: '虚', summary: 'test', heat_score: 1.0 });
      assert.strictEqual(node.isCompost(), false);
    });

    it('isCompost：支持自定义阈值', () => {
      const node = new KnowledgeNode({ node_id: 'c4', axis: '业', state: '实', summary: 'test', heat_score: 0.5 });
      assert.strictEqual(node.isCompost(0.3), false);
      assert.strictEqual(node.isCompost(0.6), true);
    });

    it('isCompost：热力为0应完全化土', () => {
      const node = new KnowledgeNode({ node_id: 'c5', axis: '生', state: '实', summary: 'test', heat_score: 0 });
      assert.strictEqual(node.isCompost(), true);
    });

    it('touch 可唤醒化土节点', () => {
      const node = new KnowledgeNode({ node_id: 'c6', axis: '业', state: '实', summary: 'test', heat_score: 0.1 });
      assert.strictEqual(node.isCompost(), true);
      node.touch(); // +0.3 → 0.4
      assert.strictEqual(node.isCompost(), false);
    });
