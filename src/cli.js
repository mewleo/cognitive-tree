...
function seedDemoData() {
  ...
  tree.addNode(new KnowledgeNode({
    node_id: 'work_oddm_path', axis: '业', state: '实',
    summary: 'ODL路径寻址即拓扑', implicit_tags: ['解耦'], heat_score: 1.8,
  }));
  tree.addNode(new KnowledgeNode({
    node_id: 'work_oddm_ref', axis: '业', state: '实',
    summary: 'ref懒引用按需加载', implicit_tags: ['解耦', '惰性'], heat_score: 1.5,
  }));
  tree.addNode(new KnowledgeNode({
    node_id: 'life_sleep', axis: '生', state: '实',
    summary: '作息调理与褪黑素影响', implicit_tags: ['生理节律', '复利'], heat_score: 1.2,
  }));
  tree.addNode(new KnowledgeNode({
    node_id: 'life_family', axis: '生', state: '实',
    summary: '家庭分工明确减少内耗', implicit_tags: ['解耦', '边界'], heat_score: 0.9,
  }));
  tree.addNode(new KnowledgeNode({
  ...
  tree.addNode(new KnowledgeNode({
    node_id: 'thought_axis', axis: '思', state: '虚',
    summary: '生业思三主干模型', implicit_tags: ['极简'], heat_score: 1.6,
  }));
  tree.addNode(new KnowledgeNode({
    node_id: 'thought_heat', axis: '思', state: '虚',
    summary: '热力驱动应季显隐', implicit_tags: ['自组织'], heat_score: 1.4,
  }));
  tree.addNode(new KnowledgeNode({
    node_id: 'thought_crystal', axis: '思', state: '虚',
    summary: '认知结晶升维第一性原理', implicit_tags: ['涌现', '极简'], heat_score: 1.3,
  }));
...
.header h1{font-size:22px;color:#a8d8ea}
.header .stats{font-size:13px;color:#888;margin-top:6px}
.tree-wrap{display:flex;justify-content:center;align-items:flex-start;padding:10px;overflow:auto}
svg{max-width:100%}
...
.axis-label{font-size:16px;font-weight:bold;fill:#a8d8ea;text-anchor:middle}
.link{stroke:#444;stroke-width:1.5;fill:none}
.cross-link{stroke:#6a5acd;stroke-width:1;stroke-dasharray:4,3;fill:none;opacity:.5}
.tooltip{position:fixed;background:#16213e;border:1px solid #0f3460;border-radius:8px;padding:12px 16px;font-size:13px;max-width:280px;pointer-events:none;z-index:100;display:none;box-shadow:0 4px 20px rgba(0,0,0,.5)}
.tooltip .t-title{font-weight:bold;color:#a8d8ea;margin-bottom:6px;font-size:14px}
.tooltip .t-row{margin:3px 0;color:#bbb}
.tooltip .t-content{margin-top:8px;padding-top:8px;border-top:1px solid #333;color:#999;line-height:1.5}
.legend{text-align:center;padding:10px;font-size:12px;color:#666}
...
const axes = ['生','业','思'];
const axisAngle = {'生':-90,'业':30,'思':150};
const axisNodes = {};
for(const a of axes) axisNodes[a] = DATA.filter(n=>n.axis===a);
const positions = {};
const nodeMap = {};
DATA.forEach(n=>nodeMap[n.node_id]=n);
for(const a of axes){
  const nodes = axisNodes[a];
  const angle = axisAngle[a] * Math.PI / 180;
  ...
  const layoutNode = (node, depth, idx, total) => {
    const baseR = 100 + depth * 110;
    const spread = total > 1 ? (idx - (total-1)/2) * 80 : 0;
    const perpAngle = angle + Math.PI/2;
    const x = CX + Math.cos(angle)*baseR + Math.cos(perpAngle)*spread;
    const y = CY + Math.sin(angle)*baseR + Math.sin(perpAngle)*spread;
    positions[node.node_id] = {x,y,node};
    const children = node.children_ids.map(cid=>nodeMap[cid]).filter(Boolean);
    children.forEach((c,i)=>layoutNode(c, depth+1, i, children.length));
  };
  roots.forEach((r,i)=>layoutNode(r, 0, i, roots.length));
}
DATA.forEach(n=>{
  n.children_ids.forEach(cid=>{
    if(positions[n.node_id]&&positions[cid]){
      const p1=positions[n.node_id], p2=positions[cid];
...
DATA.forEach(n=>{
  n.cross_links.forEach(tid=>{
    ...
    if(positions[n.node_id]&&positions[tid]){
      ...
      curve.setAttribute('d',\`M\${p1.x},\${p1.y} Q\${mx-dy*0.3},\${my+dx*0.3} \${p2.x},\${p2.y}\`);
      curve.setAttribute('class','cross-link');
...
for(const a of axes){
  const angle = axisAngle[a] * Math.PI / 180;
  const x = CX + Math.cos(angle)*60, y = CY + Math.sin(angle)*60;
  const t=document.createElementNS('http://www.w3.org/2000/svg','text');
  t.setAttribute('x',x);t.setAttribute('y',y);t.setAttribute('class','axis-label');
  t.textContent='【'+a+'】';
  svg.appendChild(t);
}
Object.values(positions).forEach(({x,y,node})=>{
  const r = 8 + node.heat_score * 3;
  const c=document.createElementNS('http://www.w3.org/2000/svg','circle');
  c.setAttribute('cx',x);c.setAttribute('cy',y);c.setAttribute('r',r);
  c.setAttribute('class','node-circle');
  if(node.state==='实'){
    c.setAttribute('fill', node.axis==='生'?'#52b788':node.axis==='业'?'#4ea8de':'#cdb4db');
    c.setAttribute('stroke','#fff');c.setAttribute('stroke-width','1');
  } else {
    c.setAttribute('fill','none');
    c.setAttribute('stroke', node.axis==='生'?'#52b788':node.axis==='业'?'#4ea8de':'#cdb4db');
    c.setAttribute('stroke-width','2');c.setAttribute('stroke-dasharray','3,2');
  }
  c.addEventListener('mouseenter',e=>showTooltip(e,node));
  ...
  label.textContent = node.summary.length>12 ? node.summary.slice(0,12)+'..' : node.summary;
...
function showTooltip(e,node){
  ...
  tooltip.innerHTML = \`<div class="t-title">\${node.summary}</div><div class="t-row">主干: \${node.axis} | 状态: \${node.state} | 层级: L\${node.level}</div><div class="t-row">热力: \${node.heat_score.toFixed(1)}</div>\${node.implicit_tags.length?'<div class="t-row">隐性标签: '+node.implicit_tags.join(', ')+'</div>':''}\${node.raw_source?'<div class="t-content">'+node.raw_source+'</div>':''}\`;
  moveTooltip(e);
}
