...
let curX=LEFT_PAD;const axisStart={};for(const a of axes){const {roots}=axisTrees[a];let totalW=0;roots.forEach(r=>totalW+=treeWidth(r)*(NODE_W+HGAP));if(totalW>0){axisStart[a]=curX;roots.forEach(r=>{const w=treeWidth(r)*(NODE_W+HGAP);layout(r,curX,0);curX+=w;});curX+=HGAP*2;}}
let maxX=0,maxY=0;Object.values(positions).forEach(p=>{maxX=Math.max(maxX,p.x+NODE_W);maxY=Math.max(maxY,p.y+NODE_H);});const SVG_W=Math.max(maxX+LEFT_PAD,800);const SVG_H=maxY+80;svg.setAttribute('width',SVG_W);svg.setAttribute('height',SVG_H);
for(const a of axes){if(axisStart[a]!==undefined){const t=document.createElementNS('http://www.w3.org/2000/svg','text');t.setAttribute('x',axisStart[a]+40);t.setAttribute('y',28);t.setAttribute('class','axis-title');t.textContent='【'+a+'】';svg.appendChild(t);}}
DATA.forEach(n=>{if(!positions[n.node_id])return;const p1=positions[n.node_id];(n.children_ids||[]).forEach(cid=>{if(positions[cid]){const p2=positions[cid];const x1=p1.x+NODE_W/2,y1=p1.y+NODE_H,x2=p2.x+NODE_W/2,y2=p2.y;const my=(y1+y2)/2;const path=document.createElementNS('http://www.w3.org/2000/svg','path');path.setAttribute('d',\`M\${x1},\${y1} C\${x1},\${my} \${x2},\${my} \${x2},\${y2}\`);path.setAttribute('class','link');svg.appendChild(path);}});});
const drawnCross=new Set();DATA.forEach(n=>{(n.cross_links||[]).forEach(tid=>{const key=[n.node_id,tid].sort().join('-');if(drawnCross.has(key))return;drawnCross.add(key);if(positions[n.node_id]&&positions[tid]){const p1=positions[n.node_id],p2=positions[tid];const x1=p1.x+NODE_W/2,y1=p1.y+NODE_H/2,x2=p2.x+NODE_W/2,y2=p2.y+NODE_H/2;const mx=(x1+x2)/2,my=(y1+y2)/2-40;const path=document.createElementNS('http://www.w3.org/2000/svg','path');path.setAttribute('d',\`M\${x1},\${y1} Q\${mx},\${my} \${x2},\${y2}\`);path.setAttribute('class','cross-link');svg.appendChild(path);}});});
Object.values(positions).forEach(({x,y,node})=>{const g=document.createElementNS('http://www.w3.org/2000/svg','g');g.setAttribute('class','node-card');const rect=document.createElementNS('http://www.w3.org/2000/svg','rect');rect.setAttribute('x',x);rect.setAttribute('y',y);rect.setAttribute('width',NODE_W);rect.setAttribute('height',NODE_H);rect.setAttribute('rx',8);rect.setAttribute('ry',8);rect.setAttribute('fill','#fff');const color=axisColor[node.axis]||'#999';if(node.state==='实'){rect.setAttribute('stroke',color);rect.setAttribute('stroke-width','2');}else{rect.setAttribute('stroke',color);rect.setAttribute('stroke-width','1.5');rect.setAttribute('stroke-dasharray','4,3');}g.appendChild(rect);const dot=document.createElementNS('http://www.w3.org/2000/svg','circle');dot.setAttribute('cx',x+14);dot.setAttribute('cy',y+18);dot.setAttribute('r',4);dot.setAttribute('fill',node.state==='实'?color:'none');dot.setAttribute('stroke',color);dot.setAttribute('stroke-width','1.5');g.appendChild(dot);const title=document.createElementNS('http://www.w3.org/2000/svg','text');title.setAttribute('x',x+26);title.setAttribute('y',y+22);title.setAttribute('class','node-text');title.textContent=node.summary.length>16?node.summary.slice(0,16)+'..':node.summary;g.appendChild(title);const tags=(node.implicit_tags||[]).slice(0,2).join('·');const heat='🔥'.repeat(Math.min(3,Math.ceil(node.heat_score)));const sub=document.createElementNS('http://www.w3.org/2000/svg','text');sub.setAttribute('x',x+10);sub.setAttribute('y',y+40);sub.setAttribute('class','node-sub');sub.textContent=(tags?tags+' ':'')+heat+(node.level>1?' L'+node.level:'');g.appendChild(sub);g.addEventListener('mouseenter',e=>showTooltip(e,node));g.addEventListener('mousemove',e=>moveTooltip(e));g.addEventListener('mouseleave',hideTooltip);svg.appendChild(g);});
...
function showTooltip(e,node){tooltip.style.display='block';tooltip.innerHTML=\`<div class="t-title">\${node.summary}</div><div class="t-row">主干: \${node.axis} | 状态: \${node.state} | 层级: L\${node.level}</div><div class="t-row">热力: \${node.heat_score.toFixed(1)}</div>\${node.implicit_tags&&node.implicit_tags.length?'<div class="t-row">隐性标签: '+node.implicit_tags.join(', ')+'</div>':''}\${node.explicit_tags&&node.explicit_tags.length?'<div class="t-row">显性标签: '+node.explicit_tags.join(', ')+'</div>':''}\${node.raw_source?'<div class="t-content">'+node.raw_source+'</div>':''}\`;moveTooltip(e);}
